...
// sudo bpftrace -e 'usdt:./src/elfview:cli:elf_load{printf("%s %s\n", probe, str(arg0));}'
#define trace_cli_elf_load(path) \
	__trace_cli_probe_s1(cli, elf_load, path)

// sudo bpftrace -e 'usdt:./src/elfview:cli:elf_delete{printf("%s %s\n", probe, str(arg0));}'
#define trace_cli_elf_delete(path) \
	__trace_cli_probe_s1(cli, elf_delete, path)

// sudo bpftrace -e 'usdt:./src/elfview:cli:elf_select{printf("%s %s\n", probe, str(arg0));}'
#define trace_cli_elf_select(path) \
	__trace_cli_probe_s1(cli, elf_select, path)

// sudo bpftrace -e 'usdt:./src/elfview:cli:elf_list{printf("%s\n", probe);}'
